...
|---|---|---|---|
| User input | `scripts/run-reference-pipeline.js`, later JUCE UI | None | Accepts reference path, output folder, similarity level, optional direction. |
...
| Similarity controls | `analysis/reference_transform.py` | None | Converts `low` through `near-identical` into ACE task type, cover strength, prompt constraints. |
...
| Traceability | `analysis/traceability.py`, `scripts/record-listen-feedback.js` | None | Writes candidate manifest and feedback JSONL. |
