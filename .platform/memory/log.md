...
---
- 2026-07-09 — commit `98ff507`: chore: update agent workflow protocol — auto-logged
...
- 2026-05-05 — commit `a66e43d`: update — auto-logged
...
2026-05-05 — full-arrangement-proxy-v1 Phase 1 — implemented Arrangement Lock blueprint fidelity, reports, transition metadata, UI summary paths, and ACE review gating — full-song generation now has a reviewable map contract before rendering.
2026-05-05 — full-arrangement-proxy-v1 beat-grid fallback — added independent librosa beat/downbeat analysis, bar-grid candidate comparison, radio-edit duration labeling, and review warnings for suspiciously sparse long-song section maps; All-In-One remains optional and currently blocked locally by NATTEN CPU backend compatibility.
2026-05-05 — full-arrangement-proxy-v1 section reliability — extracted phrase-aware section construction and report rendering; the 3:30 test reference now maps to six bar-aligned producer sections instead of a two-minute intro.
2026-05-05 — full-arrangement-proxy-v1 Docker reliability lane — added optional All-In-One-Fix Docker worker with NATTEN CPU import patch, stem reuse, CLI/runner opt-in, and bounded fallback; image/import smoke passes, but real 3:30 CPU analysis timed out at 300s so native Arrangement Lock remains the default reliable path.
2026-05-05 — full-arrangement-proxy-v1 confidence lift — added stable beat-grid confidence, phrase-grid scoring, and extended-mix 8/16/32-bar normalization; Gruuving improved 0.674→0.884 and the 3:30 reference improved 0.695→0.821.
2026-05-05 — debug: tribal percussion lost in ACE candidate — fixed root cause: drum density used unique 16th-grid positions across the whole long track, undercounting dense percussion; drum analysis now uses onset rates per bar, marks `tribal_percussion`, promotes it into prompts, and raises ACE source hold.
2026-05-05 — debug: full pipeline produced only a close short sample — fixed root cause: `--duration` controlled the sample lane while full Arrangement Lock audio was hidden behind env flags; added `--full-arrangement` to enable section stitch mode, skip the short sample lane, and default sections to medium-low inspired conditioning.
...
2026-07-01 — debug: ACE reconstruction diagnostic drifted into bright/pop output — fixed root cause: diagnostic runs still carried auto harmonic/key prompt text, upstream variation prompts still said not to copy bass pitches, and the May 24 timeout workaround stopped duplicating the source WAV as both `reference_audio` and `ctx_audio`; reconstruction diagnostic now bypasses variation language, leaves key metadata empty unless explicitly requested, and restores dual audio upload for copy-behavior testing.
2026-07-01 — debug: ACE copy-behavior still not near-identical after prompt cleanup — found backend mismatch: current Apple Silicon MLX DiT path accepts `audio_cover_strength` but does not forward/use `cover_noise_strength`, while the PyTorch/base/turbo model path implements source-latent renoising for `cover_noise_strength`; added `ACESTEP_USE_MLX_DIT=0` diagnostic startup switch to compare PyTorch DiT against MLX.
2026-07-03 — docs/config: ACE reference calibration defaults — made `npm run ace-step:start` default to `ACESTEP_USE_MLX_DIT=0`, added a setup/start patch for ACE-Step's ignored-cache env override, and documented the PyTorch/MPS settings ladder for near-copy, 50%-inspired, and far-inspired underground house proxy generation.
2026-07-03 — docs/config: successful generation recipe database — added `docs/pipelines/successful-generation-configs.md` to preserve working ACE/Suno prompts, slider settings, genre boundaries, instrumental Suno configs, sparse adult vocal direction, successful Teskera notes, and the packaging rule for main drums vs percussion reference stems.
2026-07-03 — docs/config: Suno club-tool anti-festival prompt — recorded the successful no-vocal underground club-tool prompt that uses `minimal harmonic movement`, `no emotional chord progressions`, and strict anti-trance/electro-house constraints to keep Suno away from festival melodies.
2026-07-03 — docs/config: preferred full-track Suno prompt — documented that `club tool` can cause snippet-like generations and added the preferred no-vocal full-length underground house prompt with explicit intro/groove/breakdown/drop/outro structure plus minimal-harmony constraints.
