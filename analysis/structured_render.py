...
def render_control_scaffold(
    ...
    """Render a diagnostic in-key bass/drum scaffold for ACE conditioning.

    This is not intended to be auditioned as a musical candidate. It is only a
    control signal for experiments where ACE needs source audio without the
    original bass pitches.
    """
    out = Path(output_dir)
    ...
    payload = {
        ...
        "composition": _composition_summary(composition),
        "purpose": "Diagnostic ACE cover/reference conditioning with controlled in-key bass rhythm and tone.",
        "warnings": [
            "This is an internal control signal, not a user-listenable generated track.",
            "This scaffold is generated by the pipeline and is not the original reference recording.",
            "For rich/vocal references, this can sound musically wrong and should not be treated as a final result.",
...
def _render_mix(
    ...
    for bar, step, note, dur_steps, vel in events["bass"]:
        t = _event_time(bar, step, s16, composition["swing"])
        bass_voice = _clean_bass_guide if clean_bass_guide else _bass
        _add(music, bass_voice(note, dur_steps * s16, vel), t)
...
def _clean_bass_guide(note: int, duration: float, velocity: float) -> np.ndarray:
    """Clean, rounded bass guide for ACE conditioning, not final sound design."""
    length = max(32, int(SR * duration))
    t = np.arange(length) / SR
    f = _midi_to_hz(note)
    env = _adsr(length, 0.004, 0.055, 0.72, 0.045)
    sub = np.sin(2 * np.pi * f * t)
    soft_harmonic = np.sin(2 * np.pi * f * 2.0 * t) * 0.18
    triangle = (2.0 / np.pi) * np.arcsin(np.sin(2 * np.pi * f * t)) * 0.24
    mono = _lowpass((sub * 0.78 + triangle + soft_harmonic) * env, SR, 520)
    mono = np.tanh(mono * 1.25) * 0.48 * velocity
    return _stereo(mono, width=0.02)
