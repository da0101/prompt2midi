...
from analysis.core.feature_extraction import analyze_wav
from analysis.detectors.drum_analysis import drum_pattern_to_midi_events
from analysis.detectors.vocal_analysis import analyze_vocal_role
from analysis.generation.audio_generation import _condition_prompt, _sanitize_user_prompt_for_model, _should_use_control_scaffold
from analysis.generation.full_guide_audio import generate_full_arrangement_guide_audio
from analysis.generation.providers.ace_step_generation import (
...
from analysis.composition.composition import _composition_style
...
from analysis.reference.reference_groove import _analyze_reference_groove_safe, score_groove_similarity
from analysis.midi.source_transcription import _extract_bassline, transcribe_with_model
    ...
    def test_allin1_docker_adapter_parses_success_envelope(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch.dict(os.environ, {"PROMPT2MIDI_ENABLE_ALLIN1_DOCKER": "1"}, clear=False):
                os.environ.pop("PROMPT2MIDI_DISABLE_ALLIN1", None)
                with mock.patch("analysis.core.external_analyzers.subprocess.run", return_value=completed):
    ...
    def test_full_guide_audio_uses_inspired_section_stitch_profile(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with mock.patch("analysis.generation.full_guide_audio._extract_wav"), mock.patch("analysis.generation.full_guide_audio._concat_wavs"), mock.patch(
                    "analysis.generation.full_guide_audio.generate_with_ace_step",
    ...
    def test_full_guide_audio_selects_best_passing_section_candidate(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with mock.patch("analysis.generation.full_guide_audio._extract_wav"), mock.patch(
                    "analysis.generation.full_guide_audio._concat_wavs",
                    side_effect=fake_concat,
                ), mock.patch("analysis.generation.full_guide_audio.generate_with_ace_step", side_effect=fake_generate):
                    result = generate_full_arrangement_guide_audio(
                        reference_audio="reference.wav",
                        output_dir=temp_dir,
